...
class MoEGate(nn.Module):
    def __init__(self, config):
        ...
        # 从hidden_size -> num_expert的一个计算权重
...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states):
        identity = hidden_states  # (b, s, h)
        orig_shape = hidden_states.shape  # (b, s, h)
        topk_idx, topk_weight, aux_loss = self.gate(hidden_states)  # (b*s, 2), (b*s, 2), loss
        hidden_states = hidden_states.view(-1, hidden_states.shape[-1])  # (b*s, h)
        flat_topk_idx = topk_idx.view(-1)  # (b*s*2)
        if self.training:
            hidden_states = hidden_states.repeat_interleave(self.num_experts_per_tok, dim=0)  # (b*s*2, h)
            y = torch.empty_like(hidden_states)  # (b*s*2, h)
            for i, expert in enumerate(self.experts):
                y[flat_topk_idx == i] = expert(hidden_states[flat_topk_idx == i])  # 取出对应专家的hidden送入专家计算
            # (b*s*2,h) -> (b*s, 2, -1) = (b*s, 2, h) 
            # (b*s, 2) -> (b*s, 2, 1)
            # 相乘 然后 dim=1求和
            # 整体目的就是：一个token选两个专家，那么有两个输出，这两个输出要加权求和，得到最终的输出
