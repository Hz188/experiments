# -*- coding: utf-8 -*-
# BookStorageViewer.py
# @author 灏喆
# @description 书籍信息表格
# @created 2019-05-26T15:55:29.056Z+08:00
# @last-modified 2019-06-13T11:04:03.615Z+08:00
#
...
class BookStorageViewer(QWidget):
    ...
    def __init__(self):
        ...
        self.totalRecord = 0  # 总记录数
        self.pageRecord = 10  # 每页数据数
        self.setUpUI()
    ...
    def recordQuery(self, index):
        ...
        if ("按书名查询" == conditionChoice):
            conditionChoice = 'BookName'
        elif ("按书号查询" == conditionChoice):
            conditionChoice = 'BookId'
        elif ("按作者查询" == conditionChoice):
            conditionChoice = 'Author'
        elif ('按分类查询' == conditionChoice):
        ...
        if ("" == self.searchEdit.text()):
            ...
            queryCondition = ("select * from Book ORDER BY %s  limit %d,%d " %
        ...
        for i in range(0, len(temp)):
            s = s + temp[i] + "%"
        queryCondition = (
            "SELECT * FROM Book WHERE %s LIKE '%s' ORDER BY %s " %
        ...
        self.pageLabel.setText(label)
        queryCondition = (
            "SELECT * FROM Book WHERE %s LIKE '%s' ORDER BY %s LIMIT %d,%d " %
