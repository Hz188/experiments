...
def parse_args(extra_args_provider=None, defaults={},
    ...
    # Distributed args.  # 设定分布式训练的各种参数
    args.rank = int(os.getenv('RANK', '0')) # torchrun 会设定对应进程的rank、world_size等环境变量，比如world_size=16
    args.world_size = int(os.getenv("WORLD_SIZE", '1'))
    # Tensor model parallel size.
    args.tensor_model_parallel_size = min( # 设定tensor并行度，比如2
        args.tensor_model_parallel_size, args.world_size)
    assert args.world_size % args.tensor_model_parallel_size == 0, 'world size'\
        ' ({}) is not divisible by tensor model parallel size ({})'.format(
            args.world_size, args.tensor_model_parallel_size)
    # Pipeline model parallel size.
    args.pipeline_model_parallel_size = min( # 设定model_pipeline并行度，16 / 2 最多8卡进行流水线并行，这里我们假设4，所以取4
        args.pipeline_model_parallel_size,
        (args.world_size // args.tensor_model_parallel_size))
    # Checks.
    model_parallel_size = args.pipeline_model_parallel_size * \
                          args.tensor_model_parallel_size # 整体的模型并行size是：张量*pipeline=2*4=8
    assert args.world_size % model_parallel_size == 0, 'world size is not'\
        ' divisible by tensor parallel size ({}) times pipeline parallel ' \
        'size ({})'.format(args.world_size, args.tensor_model_parallel_size,
                           args.pipeline_model_parallel_size)
    args.data_parallel_size = args.world_size // model_parallel_size # 所以16 / 8 能进行的数据并行为2
    if args.rank == 0:
