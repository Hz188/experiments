...
class ColumnParallelLinear(torch.nn.Module):
    ...
    def __init__(self, input_size, output_size, bias=True, gather_output=True,
        ...
        args = get_args()
        if args.use_cpu_initialization:
            self.weight = Parameter(torch.empty(self.output_size_per_partition,  # 用切分的size初始化权重
                                                self.input_size,
                                                dtype=args.params_dtype))
            ...
            self.weight = Parameter(torch.empty(         # 用切分的size初始化权重
        ...
        if bias:
            if args.use_cpu_initialization:
                self.bias = Parameter(torch.empty(             # 用切分的size初始化权重
