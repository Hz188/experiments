...
bool SampleOnnxMNIST::build() {
    ...
    // 5. construct the network, 在这里封装了一下config相关的过程，step in 进去
...
bool SampleOnnxMNIST::constructNetwork(SampleUniquePtr<nvinfer1::IBuilder> &builder,
    ...
    if (mParams.fp16) config->setFlag(BuilderFlag::kFP16); //BuilderFlag有很多选项提供给我们使用
...
bool SampleOnnxMNIST::infer() {
    ...
    // 0. demo给的封装，用于管理CPU和GPU内存，不用我们再手动申请了
...
int main(int argc, char **argv) {
    ...
    if (!sample.build())  // 1. trt build step
    {
